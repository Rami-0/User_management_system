...
public:
	...
	static std::string readPhoneNumber(const std::string& message = "Please enter phone number: ")
	{
		...
		while (true)
		{
			...
			if (std::cin.fail() || phoneNumber.length() != 10 || !std::all_of(phoneNumber.begin(), phoneNumber.end(), isdigit))
			{
				std::cin.clear();
				std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
				std::cerr << "Invalid input! Phone number should be a 10-digit numeric value." << std::endl;
			}
			else
			{
				std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
